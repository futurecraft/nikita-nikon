...
`docker run -it --network="host" --entrypoint=/bin/bash petstoredemo`

Now you're logged into container and can execute tests directly:
...
After the test is executed you can `exit` container session and retrieve the HTML report from the container.
...
- `'/app/results/reports/cukes_report_28.03_15:42:12.html'` is a path to the report. Search console output for 'REPORT PATH' (last line of test execution output)
